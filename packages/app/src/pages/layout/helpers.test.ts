...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("opennodus://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("opennodus://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("opennodus://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("opennodus://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("opennodus://open-project")).toBeUndefined()
    expect(parseDeepLink("opennodus://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "opennodus://open-project?directory=/a",
      "opennodus://other?directory=/b",
      "opennodus://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("opennodus://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("opennodus://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("opennodus://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("opennodus://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "opennodus://new-session?directory=/a",
      "opennodus://open-project?directory=/b",
      "opennodus://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENNODUS__: {
        deepLinks: ["opennodus://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENNODUS__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["opennodus://open-project?directory=/a"])
